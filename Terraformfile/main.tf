...
module "route_table_module" {
    ...
    vpc_id = module.vpc_modules[each.value.vpc_name].vpc_id
    internetGW_id = each.value.private == 0 ? module.internetGW_module[each.value.gateway_name].internetGW_id : module.NatGW_module[each.value.gateway_name].nat_GW_id
    tags = each.value.tags
